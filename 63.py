...
def update(frame):
    ...
    trajectory_3d = np.vstack((trajectory_x, trajectory_y, trajectory_z)).T
    reduced_trajectory = pca.fit_transform(trajectory_3d)
    
    # Plot the reduced trajectory in 3D space
    ax.clear()
    ax.plot(reduced_trajectory[:, 0], reduced_trajectory[:, 1], reduced_trajectory[:, 2])
