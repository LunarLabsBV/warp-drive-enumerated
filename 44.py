...
def update(frame):
    ...
    ax.plot(trajectory_x, trajectory_y, color='blue', alpha=0.5)  
    
    # Plot the fuselage (2D representation of the spacecraft)
    fuselage_x = np.linspace(-0.2, 0.2, 10)
    fuselage_y = np.zeros_like(fuselage_x)
    ax.plot(fuselage_x, fuselage_y, color='gray', alpha=0.8)  # Fuselage
    ax.scatter(0, 0, color='green', s=100)  # Cockpit
