...
def update(frame):
    ...
    ax.clear()
    ax.plot(trajectory_x, trajectory_y, trajectory_z, color='blue')  # Main body of the spacecraft
    ax.plot(trajectory_x, trajectory_y, np.zeros_like(trajectory_x), color='red', linestyle='dashed')  # Energy field
    ax.scatter(trajectory_x[-1], trajectory_y[-1], trajectory_z[-1], color='yellow', s=100)  # Engine
    ax.scatter(trajectory_x[0], trajectory_y[0], trajectory_z[0], color='green', s=100)  # Cockpit
