...
sandwich_size = np.zeros_like(time_step)

# Define function to animate sandwich with Reubens Factor
def update(frame):
    global sandwich_x, sandwich_y, sandwich_z, sandwich_size
    ...
    # Adjust sandwich size based on Reubens Factor
    sandwich_size = np.abs(reubens_factors) * 10
    
    # Plot sandwich
    ax.clear()
    ax.scatter(sandwich_x, sandwich_y, sandwich_z, s=sandwich_size, c=sandwich_z, cmap='viridis')
